...
do
    case "$opt" in
        ...
        o)
             THEME_VENDOR="$OPTARG"
             echo "using $THEME_VENDOR vendorsetup_task650.sh"
             ;;
        b)
             BUILDN="$OPTARG"
             ;;
...
if [ ! -f vendor/$VENDOR/vendorsetup_task650.sh ]; then
    echo "vendorsetup_task650.sh not found"
    echo "exiting..."
    exit 1
fi
...
if [[ -z "$debug" && ! -f .bot_lunch ]]; then
    ...
    cat vendor/$VENDOR/vendorsetup_task650.sh | cut -f2 -d ' ' > .bot_lunch
fi
